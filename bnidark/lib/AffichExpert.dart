...
class him extends StatelessWidget {
  const him({Key? key}) : super(key: key);
...
class _DSState extends State<DS> {
  final Stream<QuerySnapshot> _exper =
      FirebaseFirestore.instance.collection('Expertt').snapshots();
  void Addetoil(String docid, int like) {
    var newetiol = like + 1;
    try {
      FirebaseFirestore.instance.collection('Expertt').doc(docid).update({
        'likes': newetiol,
